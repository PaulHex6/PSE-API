...
def forecast_holt_winters(data, seasonal_period):
    if not data.empty:
        data['udtczas'] = pd.to_datetime(data['udtczas'])
        data.set_index('udtczas', inplace=True)
        progress_bar = st.progress(0)
        progress_bar.progress(10)
        model = ExponentialSmoothing(data['rce_pln'], seasonal_periods=seasonal_period, trend='add', seasonal='add')
        model_fit = model.fit(disp=False)
        progress_bar.progress(80)
        forecast_df = generate_forecast(model_fit, data)
        progress_bar.progress(100)
...
# Common function to generate and format forecast results
